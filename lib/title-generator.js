...
class TitleGenerator {
    ...
    /**
     * 메인 제목 생성 메서드 - 전체 파이프라인 실행
     * @returns {Promise<Object>} 제목 생성 결과
     */
    async generateTitles() {
        this.logger.logStep('제목 생성 파이프라인 시작', {});
        ...
        try {
            ...
            const processedCandidates = this.processAndRankCandidates(candidates);
            results.candidates = processedCandidates;

            // 최적 제목 선택
            results.bestTitle = processedCandidates.length > 0 ? processedCandidates[0].title : 'AI 뉴스';
