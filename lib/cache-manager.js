...
class CacheManager {
    constructor(options = {}) {
        ...
        this.titleEvaluationCache = new Map();
     ...
     */
    cacheTitleEvaluation(cacheKey, evaluationResult) {
        const cacheEntry = {
            data: evaluationResult,
            timestamp: Date.now(),
            type: 'title_evaluation',
            size: this.estimateObjectSize(evaluationResult)
        };
        
        this.setCache(this.titleEvaluationCache, cacheKey, cacheEntry);
        console.log('제목 평가 결과 캐시 저장:', cacheKey.substring(0, 16) + '...');
    }

    /**
     * 캐시된 제목 평가 결과 조회
     * @param {string} cacheKey 캐시 키
     * @returns {Object|null} 캐시된 평가 결과 또는 null
     */
    getCachedTitleEvaluation(cacheKey) {
        const cached = this.getCache(this.titleEvaluationCache, cacheKey);
        
        if (cached) {
            console.log('제목 평가 캐시 히트:', cacheKey.substring(0, 16) + '...');
            return cached.data;
        }
        
        console.log('제목 평가 캐시 미스:', cacheKey.substring(0, 16) + '...');
        return null;
    }
    ...
    cleanup() {
        ...
        const caches = [this.contentAnalysisCache, this.aiResponseCache, this.titleCandidatesCache, this.titleEvaluationCache];
    ...
    getStats() {
        const totalEntries = this.contentAnalysisCache.size + 
                           ...
                           this.titleEvaluationCache.size;
        ...
        return {
            ...
            cacheBreakdown: {
                ...
                titleEvaluation: this.titleEvaluationCache.size
            }
        };
    }
    ...
    clearCache(type = 'all') {
        ...
        if (type === 'all' || type === 'title_evaluation') {
            this.titleEvaluationCache.clear();
        }
        if (type === 'all') {
